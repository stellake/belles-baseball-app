...
const UserCard = ({ name, description, src, ig }) => (
  <li className="user-card-item">
    ...
    <div className="profile-image">
      <img width="100%" src={src || placeholder} alt={'Profile image of ' + name}/>
    </div>
    {ig && (
    ...
    <div>
      ...
      {SHOULD_SHOW_ROSTER_2022 && (
        <>
          <h2>2022 Roster</h2>
              <h3>Coaches</h3>
              <ul style={{ listStyle: 'none', display: 'flex', margin: '0 0 25px', padding: 0 }}>
                {coaches2022.map((coach) => (
                  <UserCard name={coach.name} description={coach.description} src={coach.src} />
              ))} 
              </ul>
              <h3>Players</h3>
              <ul style={{ listStyle: 'none', display: 'flex', margin: '0 0 25px', padding: 0, width: '100%', flexWrap: 'wrap' }}>
